import * as d3 from '../dependencies/d3.js'
import { isArr, isDef } from '../dependencies/jty.js'
import { sampleData } from './data.js'
import { Area2D } from './area2d.js'

/**
 * Identity function — returns its argument unchanged.
 * @template T
 * @param {T} value
 * @returns {T}
 */
function id(value) {
    return value
}

/**
 * Splits a flat guides array into separate x-axis and y-axis guide arrays.
 * @param {Array<{x?: number, label?: string}|{y?: number, label?: string}>} guides
 * @returns {{xGuides: Object[], yGuides: Object[]}}
 */
function groupGuides(guides) {
    if (!isArr(guides)) {
        throw new Error(`guides must be an array. Got ${typeof guides}: ${guides}`)
    }

    const xGuides = []
    const yGuides = []

    for (const guide of guides) {
        if (guide.x !== undefined) {
            xGuides.push(guide)
        } else if (guide.y !== undefined) {
            yGuides.push(guide)
        } else {
            throw new Error(`Invalid guide: ${JSON.stringify(guide)}`)
        }
    }

    return { xGuides, yGuides }
}

/**
 * Wraps a label-render function so that errors fall back to returning the raw value.
 * @param {function(number|string): string} fn
 * @returns {function(number|string): string}
 */
function labelRenderFallback(fn) {
    return function labelRender(value) {
        try {
            return fn(value)
        } catch (error) {
            console.error(
                `runLabelFunction(): Falling back due to error running label function ${fn.name}(${value}): ${/** @type {Error} */ (error).message}`,
            )
            return value
        }
    }
}

/**
 * D3-backed 2-D plot that renders a line or bar chart with axes, guides, and an interactive crosshair.
 * Extends {@link Area2D} for coordinate/scale management.
 */
export class Plot2dD3 extends Area2D {
    /**
     * @param {number} width SVG width in pixels.
     * @param {number} height SVG height in pixels.
     * @param {number} padding Inner padding around the plot area.
     * @param {[number, number]} xExtent [min, max] domain for the x axis.
     * @param {[number, number]} yExtent [min, max] domain for the y axis.
     * @param {boolean} [isBarChart=false] Render bars instead of a line path.
     * @param {function(number|string): string} [labelRenderX=id] Tick formatter for the x axis.
     * @param {function(number|string): string} [labelRenderY=id] Tick formatter for the y axis.
     */
    constructor(width, height, padding, xExtent, yExtent, isBarChart = false, labelRenderX = id, labelRenderY = id) {
        super(width, height, padding)
        this.setExtentX(xExtent)
        this.setExtentY(yExtent)
        this.labelRenderX = labelRenderFallback(labelRenderX)
        this.labelRenderY = labelRenderFallback(labelRenderY)
        this.isBarChart = isBarChart
        this.data = []
    }

    /**
     * Mounts the SVG structure (axes, path/bars, guides, crosshair) into the given element.
     * Must be called once before any update methods.
     * @param {SVGSVGElement} svgElement
     */
    mount(svgElement) {
        this.svg = d3.select(svgElement)

        this.svg
            .attr('viewBox', this.viewBox)
            .attr('width', this.width)
            .attr('height', this.height)
            .classed('plot-2d', true)

        // Placeholder for the data
        if (this.isBarChart) {
            // Placeholder for bar charts
            this.linesGroup = this.svg.append('g').classed('plot-2d__bars', true)
        } else {
            // Placeholder for line charts
            this.path = this.svg.append('path').classed('plot-2d__line', true)
        }

        // Create placeholder for the axes
        this.xAxisGroup = this.svg
            .append('g')
            .classed('plot-2d__axis plot-2d__axis--x', true)
            .attr('transform', `translate(0,${this.bottomSide})`)

        this.yAxisGroup = this.svg
            .append('g')
            .classed('plot-2d__axis plot-2d__axis--y', true)
            .attr('transform', `translate(${this.leftSide},0)`)

        // Add X axis label:
        this.xAxisTitle = this.svg
            .append('text')
            .classed('plot-2d__axis-title plot-2d__axis-title--x', true)
            .attr('x', this.rightSide)
            .attr('y', this.height)

        // Y axis label:
        this.yAxisTitle = this.svg
            .append('text')
            .classed('plot-2d__axis-title plot-2d__axis-title--y', true)
            .attr('y', this.topSide)
            .attr('x', 0)

        // Create placeholder for the guides
        this.guidesX = this.svg.append('g').classed('plot-2d__guides plot-2d__guides--x', true)

        this.guidesY = this.svg.append('g').classed('plot-2d__guides plot-2d__guides--y', true)

        // Create the crosshair
        this.crosshairGroup = this.svg.append('g').classed('plot-2d__crosshair', true).style('display', 'none')

        // Add a vertical line to the crosshair
        this.crosshairX = this.crosshairGroup
            .append('line')
            .classed('plot-2d__crosshair-line--vertical', true)
            .attr('x1', this.leftSide)
            .attr('y1', this.topSide)
            .attr('x2', this.leftSide)
            .attr('y2', this.bottomSide)

        // Add a horizontal line to the crosshair
        this.crosshairY = this.crosshairGroup
            .append('line')
            .classed('plot-2d__crosshair-line--horizontal', true)
            .attr('x1', this.leftSide)
            .attr('y1', this.topSide)
            .attr('x2', this.rightSide)
            .attr('y2', this.topSide)

        // Add a circle to the crosshair
        this.crosshairCircle = this.crosshairGroup
            .append('circle')
            .classed('plot-2d__crosshair-circle', true)
            .attr('r', 3) // radius of the circle

        this.crosshairXLabel = this.crosshairGroup
            .append('text')
            .classed('plot-2d__crosshair-label plot-2d__crosshair-label--x', true)
            .attr('y', this.topSide)

        this.crosshairYLabel = this.crosshairGroup
            .append('text')
            .classed('plot-2d__crosshair-label plot-2d__crosshair-label--y', true)
            .attr('x', this.rightSide)

        this.svg.on('mousemove', (event) => this.onMouseMove(event))
    }

    /**
     * Updates the chart with new data, re-draws axes, and re-renders the path or bars.
     * Automatically downsamples data when it exceeds 2× the pixel width.
     * @param {[number, number][]} [data] Array of [x, y] coordinate pairs. Omit to redraw with existing data.
     */
    updateData(data) {
        if (isDef(data)) {
            this.data = data
        }

        /*
        const xDataExtent = d3.extent(data, d => d[0])
        const xGuidesExtent = d3.extent(xGuides, g => g.x)
        const xExtent = d3.extent([...xDataExtent, ...xGuidesExtent])
        this.xScale.domain(xExtent)

        const yDataExtent = d3.extent(data, d => d[1])
        const yGuidesExtent = d3.extent(yGuides, g => g.y)
        const yExtent = d3.extent([...yDataExtent, ...yGuidesExtent])
        this.yScale.domain(yExtent)
        */

        const xAxis = d3.axisBottom(this.xScale).tickFormat(this.labelRenderX)
        const yAxis = d3.axisLeft(this.yScale).tickFormat(this.labelRenderY)

        this.xAxisGroup.call(xAxis)
        this.yAxisGroup.call(yAxis)

        const dataToDraw = sampleData(this.data, this.width * 2)
        if (dataToDraw.length !== this.data.length) {
            console.warn(`Original data length: ${this.data.length}, Drawing ${dataToDraw.length} samples`)
        }

        if (this.isBarChart) {
            this.linesGroup
                .selectAll('line')
                .data(dataToDraw)
                .join('line')
                .attr('x1', (d) => this.xScale(d[0]))
                .attr('x2', (d) => this.xScale(d[0]))
                .attr('y1', this.bottomSide) // start at the bottom of the chart
                .attr('y2', (d) => this.yScale(d[1])) // end at the data point
                .attr('class', 'plot-2d__bars')
                .append('title')
                .text((d, i) => `#${i}: [${d[0]}, ${d[1]}]`)
        } else {
            const line = d3
                .line()
                .x((d) => this.xScale(d[0]))
                .y((d) => this.yScale(d[1]))

            this.path.datum(dataToDraw).attr('d', line)
        }
    }

    /**
     * Updates the guide lines and labels for both axes.
     * @param {Array<{x?: number, y?: number, label?: string}>} [guides] Guide descriptors. Omit to redraw with existing guides.
     */
    updateGuides(guides) {
        if (isDef(guides)) {
            this.guides = guides
        }

        const { xGuides, yGuides } = groupGuides(this.guides)
        // Update the guides
        this.guidesX
            .selectAll('g')
            .data(xGuides)
            .join(
                (enter) => {
                    const group = enter.append('g').classed('plot-2d__guide', true)

                    group
                        .append('line')
                        .classed('plot-2d__guide-line', true)
                        .attr('x1', (d) => this.xScale(d.x))
                        .attr('x2', (d) => this.xScale(d.x))
                        .attr('y1', this.topSide)
                        .attr('y2', this.bottomSide)

                    group
                        .append('text')
                        .classed('plot-2d__guide-title plot-2d__guide-title--x', true)
                        .attr('x', (d) => this.xScale(d.x))
                        .attr('y', this.topSide)
                        .text((d) => this.labelRenderX(d.label))

                    return group
                },
                (update) => {
                    update
                        .select('line')
                        .attr('x1', (d) => this.xScale(d.x))
                        .attr('x2', (d) => this.xScale(d.x))

                    update
                        .select('text')
                        .attr('x', (d) => this.xScale(d.x))
                        .text((d) => this.labelRenderX(d.label))

                    return update
                },
                (exit) => exit.remove(),
            )

        this.guidesY
            .selectAll('g')
            .data(yGuides)
            .join(
                (enter) => {
                    const group = enter.append('g').classed('plot-2d__guide', true)

                    group
                        .append('line')
                        .classed('plot-2d__guide-line', true)
                        .attr('x1', this.leftSide)
                        .attr('x2', this.rightSide)
                        .attr('y1', (d) => this.yScale(d.y))
                        .attr('y2', (d) => this.yScale(d.y))

                    group
                        .append('text')
                        .classed('plot-2d__guide-title plot-2d__guide-title--y', true)
                        .attr('x', this.rightSide)
                        .attr('y', (d) => this.yScale(d.y))
                        .text((d) => this.labelRenderY(d.label))

                    return group
                },
                (update) => {
                    update
                        .select('line')
                        .attr('y1', (d) => this.yScale(d.y))
                        .attr('y2', (d) => this.yScale(d.y))

                    update
                        .select('text')
                        .attr('y', (d) => this.yScale(d.y))
                        .text((d) => this.labelRenderY(d.label))

                    return update
                },
                (exit) => exit.remove(),
            )
    }

    /**
     * Sets the text label displayed at the end of the x axis.
     * @param {string} title
     */
    updateAxisTitleX(title) {
        this.xAxisTitle.text(title)
    }

    /**
     * Sets the text label displayed at the end of the y axis.
     * @param {string} title
     */
    updateAxisTitleY(title) {
        this.yAxisTitle.text(title)
    }

    /**
     * Handles SVG mousemove events — moves the crosshair to the nearest data point.
     * Hides the crosshair when the pointer leaves the plot area.
     * @param {MouseEvent} event
     */
    onMouseMove(event) {
        const [mouseX, mouseY] = d3.pointer(event)

        // Check if the mouse cursor is within the diagram area
        if (!this.isInside(mouseX, mouseY)) {
            // Hide the crosshair when the mouse cursor is outside the diagram area
            this.crosshairGroup.style('display', 'none')
            return
        }

        // Show the crosshair when the mouse enters the SVG
        this.crosshairGroup.style('display', null)

        const mouseXDiagram = this.xScale.invert(mouseX)

        // Find the nearest data point
        const bisect = d3.bisector((d) => d[0]).left
        const i = bisect(this.data, mouseXDiagram, 1)
        const point1 = this.data[i - 1]
        const point2 = this.data[i]
        // Closest point to the mouse cursor
        const closestPoint = mouseXDiagram - point1[0] > point2[0] - mouseXDiagram ? point2 : point1

        const [closestPointX, closestPointY] = closestPoint
        const cx = this.xScale(closestPointX)
        const cy = this.yScale(closestPointY)

        this.crosshairX.attr('x1', cx).attr('x2', cx)

        this.crosshairY.attr('y1', cy).attr('y2', cy)

        this.crosshairXLabel.attr('x', cx).text(this.labelRenderX(closestPointX))

        this.crosshairYLabel.attr('y', cy).text(this.labelRenderY(closestPointY))

        this.crosshairCircle.attr('cx', cx).attr('cy', cy)
    }
}
