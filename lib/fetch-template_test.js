...
test('_getFileNameRoot()', () => {
    ...
    const importMetaUrlWeirdCase = 'http://slc.alexewerlof.com/components/burn.Js'
    assertEquals(_getFileNameRoot(importMetaUrlWeirdCase), 'http://slc.alexewerlof.com/components/burn')
    assertThrows(
        () => _getFileNameRoot('http://slc.alexewerlof.com/components/burn'),
        Error,
        'importMetaUrl must end with ".js"',
    )
})
