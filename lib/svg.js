 ...
 * @param {...[number, number]} coordinates an array of coordinates. Each coordinate is an array like [x, y].
 * @returns {string} a string like 'x1,y1 x2,y2 x3,y3'
 */
export function arrToPolygonPoints(...coordinates) {
    ...
    return coordinates
        .map(function (coordinate) {
            if (!isArr(coordinate)) {
                throw new Error(`Each coordinate is supposed to be an array. Got ${coordinate}`)
            }
