...
import { numL10n, strFallback, entity2symbol, hasComparators, numUnitL10n, boundCaption, oppositeBound, namify } from "./fmt.js"
...
test('entity2symbol()', () => {
    ...
    assertThrows(() => entity2symbol(''), Error('Empty string: '))
    assertThrows(() => entity2symbol(undefined), Error('Unknown HTML entity: undefined'))
...
test('oppositeBound()', () => {
    assertEquals(oppositeBound('eq'), 'ne')
    assertEquals(oppositeBound('ne'), 'eq')
    ...
    assertThrows(() => oppositeBound(''), Error(`Invalid bound: `))
    assertThrows(() => oppositeBound('foo'), Error(`Invalid bound: foo`))
})
