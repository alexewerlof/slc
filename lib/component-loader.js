...
import { isDef, isFn, isInArr, isObj, isStr, isStrLen } from './validation.js'
...
export function parseComponentSpec(componentSpec, resolve) {
    ...
    let baseUrlStr = relativeUrlBase
    if (isDef(resolve)) {
        if (!isFn(resolve)) {
            throw new Error(`Expected a resolve function. Got ${resolve} (${typeof resolve})`)
        }
        baseUrlStr = resolve(baseUrlStr)
    }
    if (!isStr(baseUrlStr)) {
