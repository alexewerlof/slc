import { defineAsyncComponent } from '../dependencies/vue.js'
import { isFn, inArr, isObj, isStr, isStrLen } from '../dependencies/jty.js'
...
function loadCss(cssUrlStr) {
    return new Promise((resolve, reject) => {
        if (!cssUrlStr || inArr(cssUrlStr, loadCss.loaded)) {
            resolve()
            return
        }
...
async function loadComponent(urlStrs) {
    try {
        ...
        mod.template = mod.template ?? template
...
export function parseComponentSpec(componentSpec, resolve) {
    if (!isStr(componentSpec)) {
        throw new TypeError(`Invalid componentSpec: ${componentSpec} (${typeof componentSpec})`)
    }
    if (!isFn(resolve)) {
        throw new TypeError(`Invalid resolve: ${resolve} (${typeof resolve})`)
    }
    const lastDot = componentSpec.lastIndexOf('.')
    if (lastDot === -1) {
        throw new Error(`Invalid componentSpec: ${componentSpec}`)
    }
    const flags = componentSpec
        .slice(lastDot + 1)
        .trim()
        .toLowerCase()
    if (flags.length <= 1) {
    ...
    if (!(hasHtml || hasJs || hasCss)) {
        throw new Error(`The component at ${baseUrlStr} does not have HTML, JS, or CSS`)
    }
    ...
    return {
        ...
        urlStrs: {
            html: hasHtml ? baseUrlStr + '.html' : undefined,
            js: hasJs ? baseUrlStr + '.js' : undefined,
            css: hasCss ? baseUrlStr + '.css' : undefined,
        },
    }
}
...
async function registerComponent(app, componentSpec, resolve) {
    ...
    if (isAsync) {
        ...
        app.component(
            name,
            defineAsyncComponent(() => loadComponent(urlStrs)),
        )
    } else {
...
export async function registerComponents(app, componentSpecs, resolve) {
    await Promise.all(componentSpecs.map((componentSpec) => registerComponent(app, componentSpec, resolve)))
}
