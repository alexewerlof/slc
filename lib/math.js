import { isArr, isInt, isNum } from './validation.js'
...
/**
 * Removes an element from the array and returns the next index for a selector
 * @param {any[]} arr an array
 * @param {number} idx the index of the selected element
 * @returns {number} the index of another element or -1 if not applicable
 */
export function nextIndex(arr, idx) {
    if (!isArr(arr)) {
        throw new TypeError(`nextIndex(): "arr" should be an array. Got: ${arr} (${typeof arr})`)
    }
    if (!isInt(idx)) {
        throw new TypeError(
            `nextIndex(): Invalid array index. Got: ${idx} (${typeof idx})`,
        )
    }
    if (arr.length === 0) {
        // no op
