...
import { deflate, inflate } from '../dependencies/pako.js'
...
import { isFn, isInstance, isStr } from '../dependencies/jty.js'

/**
 * Deserialises a URL (any supported version) back into a state object.
 * @param {URL} url
 * @returns {{ urlVer: number, state: Object }}
 */
export function urlToState(url) {
    ...
    switch (urlVer) {
        ...
        case 0:
            return {
                urlVer: 4,
                state: convertV0toV3(parseUrlV0(url)).state,
            }
        case 1:
            return {
                urlVer: 4,
                state: convertV1toV3(parseUrlV1(url)).state,
            }
        case 2:
            return {
                urlVer: 4,
                state: convertV2toV3(parseUrlV2(url)).state,
            }
        case 3:
            return {
                urlVer: 4,
                state: parseUrlV3(url).state,
            }
        case 4: {
            const stateBase64 = url.searchParams.get('state')
            const binaryString = atob(stateBase64)
            const uint8Array = Uint8Array.from(binaryString, (char) => char.charCodeAt(0))
            const json = inflate(uint8Array, { to: 'string' })
            return {
                urlVer: 4,
...
/**
 * Serialises the current state into the current page URL and returns the URL string.
 * @param {Object} state
 * @returns {string}
 */
...
/**
 * Deserialises the current page URL back into a state object.
 * @returns {Object}
 */
...
/**
 * Fetches a text resource from the given URL.
 * @param {string|URL} url
...
/**
 * Parses a state from a JSON object literal, a URL string, or a YAML string.
...
export async function parseStateString(stateString) {
    ...
    const YAML = await import('../dependencies/yaml.js')
...
/**
 * Saves text content using the File System Access API, falling back to a browser download.
 * @param {string} content
 * @param {string} fileName
 * @param {string} [contentType='text/plain']
 * @returns {Promise<boolean>} true if saved via File Picker, false if aborted
 */
