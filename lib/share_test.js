...
Deno.test('An example event-based URL v0', () => {
...
Deno.test('An example time-based URL v0', () => {
...
Deno.test('An example event-based URL from v1', () => {
...
Deno.test('An example time-based URL from v1', () => {
    ...
    assertEquals(urlToState(url), {
        ...
        lowerThreshold: 0,
        upperThreshold: 2000,
        valid: 'customer records undefined',
        slo: 99,
        windowDays: 30,
        estimatedValidEvents: 1000000,
        badEventCost: 0,
...
Deno.test('An example event-based URL v2', () => {
    const url = 'https://slc.alexewerlof.com/index.html?urlVer=2&title=An+example+SLI&description=Just+a+test+to+see+how+things+work&metricName=response_code+%3D%3D+200&lowerBound=&upperBound=&metricUnit=&lowerThreshold=0&upperThreshold=2000&valid=all+requests&slo=99&windowDays=30&estimatedValidEvents=12345678&badEventCost=2&badEventCurrency=SEK&burnRate=14.4&longWindowPerc=2&shortWindowDivider=12'
...
Deno.test('An example time-based URL v2', () => {
    const url = 'https://slc.alexewerlof.com/index.html?urlVer=2&title=An+example+time-based+SLI&description=Just+a+test+to+see+how+a+time-based+SLI+is+shared&metricName=P99%28request.latency%29&metricUnit=ms&lowerBound=&upperBound=lt&lowerThreshold=0&upperThreshold=2000&valid=customer+records+undefined&slo=99&windowDays=30&estimatedValidEvents=1000000&badEventCost=0&badEventCurrency=USD&burnRate=1&longWindowPerc=10&shortWindowDivider=12'
