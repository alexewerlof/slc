...
/**
 * Normalises the probability values of an array of buckets so they sum to 100.
...
export function createBuckets(min, max, percentages) {
    ...
    return normalizeBucketProbabilities(
...
/**
 * Generates an array of random numbers sampled from the given buckets.
 * @param {number} count number of data points to generate
 * @param {Bucket[]} bucketsArr the bucket definitions
 * @param {boolean} onlyInt when true, rounds each generated value to the nearest integer
 * @returns {number[]}
 */
