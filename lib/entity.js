import { isDef, isInt, isObj, isStr, isStrLen } from '../dependencies/jty.js'
...
export class Entity {
    ...
    constructor(prefix, hasDispDesc = false) {
        ...
        if (!this._icon) {
            throw new Error(`No icon found for class ${this._className}. Please add an icon to the unicodeSymbol map.`)
        }
    }
    ...
    get markdownDisplayName() {
        return [this.icon, this.hasValidDisplayName ? this.displayName : this.id].join(' ')
    }
