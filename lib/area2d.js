...
import { isArr, isEqualArr } from '../dependencies/jty.js'
...
export class Area2D {
    ...
    constructor(width, height, padding = [0, 0, 0, 0], cartesian = false) {
        ...
        this.yScale = d3
            .scaleLinear()
            .range(cartesian ? [this.topSide, this.bottomSide] : [this.bottomSide, this.topSide])
    }
    ...
    setExtentX(arr) {
        const shouldUpdate = !isEqualArr(this.xScale.domain(), arr)
        if (shouldUpdate) {
    ...
    setExtentY(arr) {
        const shouldUpdate = !isEqualArr(this.yScale.domain(), arr)
        if (shouldUpdate) {
