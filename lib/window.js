...
export class Window {
    ...
    timeslice
    constructor(sec, valid, timeslice) {
        if (!isNum(sec)) {
        ...
        if (!isStr(valid)) {
            throw new TypeError(`Window: unit must be a number or a string. Got ${ valid }`)
        }
        this.valid = valid
    ...
    /** Normalized unit */
    get eventUnitNorm() {
        return this.isTimeBased ? humanTimeSlices(this.timeslice) : this.valid
    }
