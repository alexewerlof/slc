...
import { numL10n, numUnitL10n } from "./fmt.js"
...
export function getTimeSlices(rawSeconds, timeslice) {
    ...
    if (!inRangePosInt(timeslice, config.timeslice.min, config.timeslice.max)) {
        throw new RangeError(`Cannot calculate number of timeslices because timeslicesSec is is out of range: ${ timeslice }`)
    }
...
export function humanTimeSlices(timeslice) {
    ...
    switch (timeslice) {
        ...
        default:
            if (timeslice % 60 === 0) {
                return numUnitL10n(timeslice / 60, 'm_timeslices')
            }
            return numUnitL10n(timeslice, 's_timeslices')
    }
}