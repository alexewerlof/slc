...
Task describes why the consumer uses a service. This essentially establishes a usage link from consumer to one or more services. The reason the consumer task is important in the context service level is because one of the biggest pitfalls of implementing service levels: to measure the vanity metrics! Service level should always be measured from the perspective of the consumers. Task describes that aspect.
